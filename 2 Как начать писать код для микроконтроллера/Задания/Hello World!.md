...
17. Убедиться, что адреса переменных соответствуют ожидаемым. Для этого обратитесь к схеме секций памяти в [[Память в ВПО|статье]] и сопоставьте выведенные адреса с соответствующими секциями в памяти. Например, ```stack_variable``` лежит в конце раздела для SRAM
