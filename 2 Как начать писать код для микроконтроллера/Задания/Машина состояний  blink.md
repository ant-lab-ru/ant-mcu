...
const uint LED_BLINK_PERIOD_US = 500000;
...
> [!ATTENTION] Подсказка
> Чтобы реализовать неблокирующее мигание, вам придется созранять метку времени и использовать время микроконтроллера. Пример логики регулярного неблокирующего вызова: 
> ``` c
...
11. в `main.c` включить заголовочный файл `led-task`
12. в `main.c` добавить инициализацию и обработчик `led-task`
13. в `main.c` добавить функции колбэков:
	- `led_on_callback`
	- `led_off_callback`
	- `led_blink_callback`
14. в `main.c` в `device_api` добавить команды с описаниями и колбэками:
	- on
	- off
	- blink

15. скомпилировать прошивку, загрузить в RP2040, проверить работоспособность. На отправленную команду `version` в COM-порт, RP2040 должен возвращать имя устройства и версию прошивки. На команды `on` и `off` включать и выключать светодиод. На `blink` светодиод должен начать мигать.
