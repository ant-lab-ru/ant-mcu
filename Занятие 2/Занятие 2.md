...
- Импортировать библиотеку для работы с COM-портом ```pyserial```  и библиотеку  ```sys``` для введения команд не через терминал PUTTY, а через  терминал python в VScode:
``` python
import serial, sys
```
